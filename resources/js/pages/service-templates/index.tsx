...
const breadcrumbs: BreadcrumbItem[] = [
    { title: 'قوالب الخدمات', href: '/service-templates' },
];
...
export default function ServiceTemplatesIndex({ templates, branches, filters }: Props) {
    const [formOpen, setFormOpen] = useState(false);
    const [editingTemplateId, setEditingTemplateId] = useState<number | null>(null);
    const editingTemplate = editingTemplateId
        ? (templates.data.find((t) => t.id === editingTemplateId) ?? null)
        : null;

    const [managingTemplateId, setManagingTemplateId] = useState<number | null>(null);
    const managingTemplate = managingTemplateId
        ? (templates.data.find((t) => t.id === managingTemplateId) ?? null)
        : null;

    const [deletingTemplateId, setDeletingTemplateId] = useState<number | null>(null);
    const deletingTemplate = deletingTemplateId
        ? (templates.data.find((t) => t.id === deletingTemplateId) ?? null)
        : null;
            ...
            },
            {
                ...
                cell: (template) => (
                    <span className="text-sm text-muted-foreground">
                        {template.branches?.length ?? 0} فرع
                    </span>
                ),
            },
            {
                ...
                cell: (template) => (
                    <div className="flex items-center gap-1.5">
                        <Button
                            ...
                            size="sm"
                            title="إدارة الفروع"
                            onClick={() => setManagingTemplateId(template.id)}
                        >
